...
var _last_click_state: bool = false
...
func _simulate_mouse_click() -> void:
	if _active_minigame == null or not _cursor_enabled:
		return
	var grab_pressed := Input.is_action_pressed("mg_grab")
	var interact_pressed := Input.is_action_pressed("interact")
	var is_pressed := grab_pressed or interact_pressed
	if is_pressed == _last_click_state:
		return
	_last_click_state = is_pressed
	_inject_mouse_button(get_viewport().get_mouse_position(), is_pressed)
